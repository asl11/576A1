    ...
    def calculate_loss(self, X, y):
        ...
        for line in y:
            if line == 0: #female
                ...
                newY.append([0,1])
        data_loss = metrics.mean_squared_error(newY, self.probs)
    ...
    def backprop(self, X, y):
        ...
        for line in y:
            if line == 0:  # female
                ...
                newY.append([0, 1])
        d2 = 2 * (self.probs - newY) * self.diff_actFun(self.z2, 'Sigmoid')
        dW2 = np.transpose(d2.T @ self.a1)
        ddb2 = np.ones((self.a1.shape[0], 1))
        db2 = np.transpose(d2.T @ ddb2)

        d1 = d2 @ self.W2.T * (self.diff_actFun(self.z1, 'Sigmoid'))
        dW1 = np.transpose(d1.T @ X)
        db1 = np.transpose(d1.T @ np.ones((X.shape[0], 1)))
